...
import { downloadFilesAsZip, shareFilesNative } from '../services/fileUtils';
...
export const Gallery: React.FC<GalleryProps> = ({ photos, onReset, userImage }) => {
  ...
  const toggleSelection = (id: string) => {
    ...
    if (newSet.has(id)) {
      ...
      if (newSet.size >= 30) {
        alert("You can only select up to 30 photos.");
        return;
      }
      newSet.add(id);
    }
  ...
  const handleDownload = async () => {
    ...
    // Multiple files -> ZIP
    await downloadFilesAsZip(targets, `eventmatch-${targets.length}-photos.zip`);
  };
  ...
  const handleShare = async () => {
    ...
    await shareFilesNative(targets);
  };
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 bg-card border-b border-gray-800 sticky top-0 z-30">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-center sm:justify-end">
          <button
            ...
            className={`px-4 py-2 border border-gray-600 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${selectedIds.size === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-700 text-white'}`}
          >
          ...
          <button
            ...
            className={`px-5 py-2 hover:bg-primary/90 text-white rounded-lg font-semibold text-sm transition-colors flex items-center gap-2 ${selectedIds.size > 0 ? 'bg-primary' : 'bg-green-600 hover:bg-green-500'}`}
          >
          ...
          {isSelectionMode ? (
            ...
            <>
              ...
              <button
                onClick={onReset}
                className="px-4 py-2 bg-gray-800 hover:bg-red-500/10 hover:text-red-400 text-gray-400 rounded-lg text-sm transition-colors"
              >
                Exit
              </button>
            </>
